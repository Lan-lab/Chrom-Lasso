...
{
    string randomSampleFile= "randomSamples_" + jobChr_fun;
    ofstream randomSamples(randomSampleFile);
	...
	//ofstream outDistMatrix(distMatrixFile);
        ...
        for (int rowIt=0; rowIt<siteNum; ++rowIt)
        {
            for (int colIt=rowIt+1; colIt<siteNum; ++colIt)
            {
                if(csInterFreqMap[rowIt][colIt]>TESTFREQTHRES && abs(sitesMap[rowIt]-sitesMap[colIt])>=20000 && rand()*1.0/(RAND_MAX*1.0)<0.001)
                {
			...
			randomSamples<<domainIt<<"\t"<<csInterFreqMap[rowIt][colIt]<<"\t"<<beta_lm(1)<<"\t"<<fitPval<<"\n";
